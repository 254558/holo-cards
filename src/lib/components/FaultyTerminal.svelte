...
<script>
  ...
  onMount(() => {
    const root = host
    if (!root) return
    if (!window.WebGLRenderingContext) { mark('error', 'no-webgl'); return }

    const reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches

    // 状态标记（data-bg-started/ready/error/size/stats）
    function mark(key, val) {
      try { root.setAttribute('data-bg-' + key, val === undefined ? '1' : String(val)) } catch (e) {}
    }
    mark('started')
    ...
    try {
      ...
      const TINT = (function hexToRgb(h) {
        h = h.replace('#', '').trim()
...
`
      ...
      offs.push(() => window.removeEventListener('resize', resize))
      ...
      function sampleOnce() {
        ...
        for (let k = 0; k < pts; k++) {
          const px = Math.floor((k + 0.5) / pts * w)
    ...
    // 清理：卸载时停 rAF / ResizeObserver / window 监听
    return () => {
...
<div class="bg-terminal" bind:this={host} aria-hidden="true"></div>
